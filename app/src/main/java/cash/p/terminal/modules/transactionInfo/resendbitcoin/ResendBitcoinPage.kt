...
class ResendBitcoinPage(val input: Input) : HSPage() {
    ...
    override fun GetContent(navigation: HSNavigation) {
        // Guards against the rare race of TransactionInfoPage having left the back stack
        // (e.g. app closed) while this screen is still showing.
        val transactionInfoPageOnBackStack = remember(navigation.lastOrNull()) {
            navigation.backStack.any { it is TransactionInfoPage }
        }
        if (!transactionInfoPageOnBackStack) {
            navigation.navigateUpSafely()
            return
        }

        val transactionInfoViewModel = viewModel<TransactionInfoViewModel>(
            viewModelStoreOwner = navigation.viewModelStoreOwnerForPage(TransactionInfoPage::class)
        )
