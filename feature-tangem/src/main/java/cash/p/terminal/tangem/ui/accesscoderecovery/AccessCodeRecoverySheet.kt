...
class AccessCodeRecoverySheet(val input: Input) : HSBottomSheet() {

    @Composable
    override fun GetContent(navigation: HSNavigation) {
        val viewModel = koinViewModel<AccessCodeRecoveryViewModel>()
        LaunchedEffect(Unit) {
            viewModel.enabledDefaultState = input.recoveryEnabled
            viewModel.setEnabled(input.recoveryEnabled)
        }
        LaunchedEffect(viewModel.success.value) {
            if (viewModel.success.value) {
                navigation.setResult(this@AccessCodeRecoverySheet, Result(viewModel.enabled.value))
        ...
        AccessCodeRecoveryScreen(
            ...
            onCloseClick = { navigation.navigateUp() }
        )
    }
